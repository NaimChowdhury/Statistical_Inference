...
The appropriate testing procedure would be the $F$-test. With the $F$-test, we can asses whether or not the expected value between several groups are different from each other. A low $F$-statistic would imply that the ratio of explained variance to unexplained variance is close to 0, thus the perceived relationship we found in the previous $t$-test is due to unexplained variance.

...
There are two $p$-values and associated terms which lead to the rejection of the null hypothesis under the $t$-test with 95% confidence level. Those variables are $y$-intercept $\beta_0$, and the coefficient $\beta_17$.
...
This corresponds to a Type II error, or a false-positive. 

### 3b.

The appropriate testing procedure would be the $F$-test. With the $F$-test, we can asses whether at least one predictor has a strong relationship with the response variable. A low $F$-statistic would imply that the ratio of variance esxplained by our model to unexplained variance is close to 0, thus the perceived relationship we found in the previous $t$-test is due to unexplained variance.
...
With such an incredibly small value for the $F$-statistic, we should be able to say that none of the predictors have a strong relationship with the response variable. Unfortunately, we cannot, since with such a high p-value, we fail to reject the null hypothesis. This seems to indicate that the way in which the data was generated was not truly random.

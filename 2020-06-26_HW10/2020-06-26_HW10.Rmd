...
```{r}
## hat-values:
head(hatvalues(lm.obj))
```

Of this small set of points, 6 has the highest hat value,
...
Both $crim$ and $lstat$ seem to have a non-linear relationship with $medv$. We can try to capture those using a polynomial regression.

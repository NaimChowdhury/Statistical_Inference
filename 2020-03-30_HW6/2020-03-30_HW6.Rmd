...
my.simple.lm <- function(x,y){
  ...
  beta1 <- ( sum( (x-x.bar)*(y-y.bar) ) )/ sum( (x-x.bar)^2 ) # compute beta1
...
and afterwards calculate the % of times (out of 1000 generated confidence intervals) that the true population values $\beta_0 = 2$ and $\beta_1 = 3$ ended upw ithin their respective confidence intervals. Are those %'es equal to what we expected? Why? __Hint__: Recall the practical interpretation of a 90% confidence interval.

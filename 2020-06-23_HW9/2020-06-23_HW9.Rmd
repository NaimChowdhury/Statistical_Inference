...
hostel <- read_csv(file.path(data.dir, "hostel.csv"))
```
...
hostel.df <- hostel %>% 
  select(price.from, summary.score, rating.band, City, cleanliness, valueformoney) %>% 
  filter(City == 'Tokyo' | City == 'Osaka') %>% 
  drop_na()
```


```{r}
lm.obj <- lm(valueformoney ~ price.from + City + price.from:City, hostel.df)

summary(lm.obj)
```

```{r}
ggplot(hostel.df, aes(x = price.from, y = summary.score, col = City)) +
  geom_smooth(method = 'lm')
```
